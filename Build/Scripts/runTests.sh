...
#
# sudhaus7/fe-data-history test runner based on docker/podman.
#
if [ "${CI}" != "true" ]; then
...
loadHelp() {
    ...
    read -r -d '' HELP <<EOF
sudhaus7/fe-data-history test runner. Execute acceptance, unit, functional and other test
...
Options:
    -s <...>
        Specifies which test suite to run
            ...
            - cleanRenderedDocumentation: cleanup rendered documentation
            - composer: "composer" with all remaining arguments dispatched.
            - composerUpdate: "composer update", handy if host has no PHP
            - composerValidate: "composer validate"
            - functional: functional tests
            - lintPhp: PHP linting
            - renderDocumentation: Render documentation using php-renderer
...
Examples:
    ...
    ./Build/Scripts/runTests.sh -x -p 8.3 -- -v --filter canRetrieveValueWithGP
...
COMPOSER_ROOT_VERSION="3.x.x-dev"
...
SUFFIX=$(echo $RANDOM)
NETWORK="sudhaus7-fedatahistory-${SUFFIX}"
...
if [ "${CONTAINER_BIN}" == "docker" ]; then
    ...
    CONTAINER_COMMON_PARAMS="${CONTAINER_INTERACTIVE} ${CI_PARAMS} --rm --network ${NETWORK} -v ${ROOT_DIR}:${ROOT_DIR}${VOLUME_MOUNT_SUFFIX} -w ${ROOT_DIR}"
    CONTAINER_SIMPLE_PARAMS="${CONTAINER_INTERACTIVE} ${CI_PARAMS} --rm -v ${ROOT_DIR}:${ROOT_DIR}${VOLUME_MOUNT_SUFFIX} -w ${ROOT_DIR}"
    DOCUMENTATION_COMMON_PARAMS="${CONTAINER_INTERACTIVE} ${CI_PARAMS} --rm -v ${ROOT_DIR}:${ROOT_DIR}${VOLUME_MOUNT_SUFFIX} -v ${ROOT_DIR}:/project"
fi
...
case ${TEST_SUITE} in
    ...
    composerUpdate)
        cleanComposer
        stashComposerFiles
        ${CONTAINER_BIN} run ${CONTAINER_SIMPLE_PARAMS} --name composer-command-${SUFFIX} -e COMPOSER_CACHE_DIR=.Build/.cache/composer -e COMPOSER_ROOT_VERSION=${COMPOSER_ROOT_VERSION} ${IMAGE_PHP} /bin/bash -c "
          composer require --no-ansi --no-interaction --no-progress --no-install typo3/minimal:^${CORE_VERSION} || exit 1
          composer update --no-progress --no-interaction || exit 1
          composer info --installed typo3/cms-core ${CORE_VERSION}.*.* || exit 1
        "
        SUITE_EXIT_CODE=$?
        restoreComposerFiles
        ;;
    composerValidate)
        ${CONTAINER_BIN} run ${CONTAINER_SIMPLE_PARAMS} --name composer-command-${SUFFIX} -e COMPOSER_CACHE_DIR=.Build/.cache/composer -e COMPOSER_ROOT_VERSION=${COMPOSER_ROOT_VERSION} ${IMAGE_PHP} composer validate --no-check-lock
        SUITE_EXIT_CODE=$?
        ;;
    functional)
        ...
        case ${DBMS} in
            ...
            sqlite)
                ...
                CONTAINERPARAMS="-e typo3DatabaseDriver=pdo_sqlite --tmpfs ${ROOT_DIR}/.Build/public/typo3temp/var/tests/functional-sqlite-dbs/:rw,noexec,nosuid"
                ${CONTAINER_BIN} run ${CONTAINER_COMMON_PARAMS} --name functional-${SUFFIX} ${XDEBUG_MODE} -e XDEBUG_CONFIG="${XDEBUG_CONFIG}" ${CONTAINERPARAMS} ${IMAGE_PHP} "${COMMAND[@]}"
                SUITE_EXIT_CODE=$?
                ;;
        esac
    ...
    phpstanGenerateBaseline)
        PHPSTAN_CONFIG_FILE="Build/phpstan/Core${CORE_VERSION}/phpstan.neon"
        PHPSTAN_BASELINE_FILE="Build/phpstan/Core${CORE_VERSION}/phpstan-baseline.neon"
        COMMAND=(php -dxdebug.mode=off .Build/bin/phpstan analyse -c ${PHPSTAN_CONFIG_FILE} --no-progress --no-interaction --memory-limit 4G --allow-empty-baseline --generate-baseline=${PHPSTAN_BASELINE_FILE})
        ...
        ;;
    unit)
        PHPUNIT_CONFIG_FILE="Build/phpunit/UnitTests-${CORE_VERSION}.xml"
